...
function Header() {
    ...
    async function login() {
        ...
        try {
            ...
            if(user.data.success === true) {
                setUserData({email: user.data.user.email, name: user.data.user.name, id: user.data.user.id})
  ...
  return (
      <header
          className="fixed border-b-2 px-5 flex justify-between top-0 dark:text-white left-0 right-0 z-10 bg-opacity-10 bg-white bg-transparent dark:bg-black backdrop-filter backdrop-blur-md p-2">
          ...
          <div className='flex justify-center items-center pr-2'>
              ...
              {isLogin.id ?
                  <p onClick={logout} className='mx-2 ml-3 cursor-pointer'><LuLogOut color='red' size="1.4rem"/></p> :
                  <div>
