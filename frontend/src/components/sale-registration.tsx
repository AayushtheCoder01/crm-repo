...
import {pageLoading, ProductsAtom} from '../store/store'
import {useRecoilState, useRecoilValue} from 'recoil'
import {Checkbox} from "./ui/checkbox.tsx";
import {sendPurchaseMail} from "../functions/Mail.ts";
...
export function SaleRegistrationComponent() {
  ...
  const [isPageLoading, setPageLoading] = useRecoilState(pageLoading)
  ...
  const handleSubmit = async(e: React.FormEvent) => {
    e.preventDefault()
    setPageLoading(true)
    const api = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/sale/create`, {
      customername: name,
    ...
    if((await api).data.success === true) {
      if(sendMail) {
        await sendPurchaseMail({name: name, email: email, totalPrice: calculateTotal()})
      }
