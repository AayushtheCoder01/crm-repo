...
function CustomerDetails() {
    ...
    async function getCustomerFn() {
        ...
        try {
            ...
            const customer = await axios.get(`${Backend_URL}/api/v1/customer/get/${id}`, {
                headers: {
    ...
    //     const Backend_URL = import.meta.env.VITE_BACKEND_URL
    //     try {
    //         const customer = await axios.put(`${Backend_URL}/api/v1/customer/update/${id}`, customerData, {
    //             headers: {
    //                 Authorization: localStorage.getItem("Authorization")
    //             }
    //         })
                ...
                <div className='h-auto w-[80%] lg:w-[50%] rounded-lg pt-3 flex flex-col border-2'>
                    ...
                    <div className="flex flex-wrap items-center justify-center w-full p-3 pt-5">
                        ...
                        <div className='flex items-start font-semibold flex-col p-3 min-w-12/12'>
                            ...
                            <Input type={"number"} value={customerData.number} onChange={(e:any) =>{if (editable){setCustomerData({...customerData, number: parseInt(e.target.value)})}}} className={'w-full'}/>
                        </div>
                        ...
                        <div className='flex items-start font-semibold flex-col p-3 min-w-6/12'>
                            ...
                            <Input value={customerData.tpv} onChange={(e:any) =>{if (editable){setCustomerData({...customerData, tpv: parseInt(e.target.value)})}}} className={'w-full'}/>
                        </div>
                        <div className='flex items-start font-semibold flex-col p-3 min-w-6/12'>
                            <Label className={'pl-1'}>Customer Since</Label>
                            <Input value={new Date(customerData.createdAt).toLocaleString()} className={'w-full'}/>
                        </div>
                    ...
                    <div className='flex w-full justify-center mb-5'>
                        {
                            editable? <Button>Save</Button> : null
                        }
                    </div>
                </div>
